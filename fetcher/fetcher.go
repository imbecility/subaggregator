...
type Result struct {
	...
	Err     error
	ErrKind ErrorKind
	// Заполняются только при ErrKind == KindNoNodes
	BodyPreview string
	RawPreview  string
	ContentType string
}
...
func (s *Stats) Record(kind ErrorKind, subURL string, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.counts[kind]++
	if _, exists := s.samples[kind]; !exists {
		s.samples[kind] = fmt.Sprintf("%s → %v", subURL, err)
	}
}
...
func classifyError(err error) ErrorKind {
	...
	// Сначала проверяем наш кастомный тип
	var nnErr *parser.NoNodesErr
...
func newDomainLimiter(perDomain int) *domainLimiter {
	return &domainLimiter{sems: make(map[string]chan struct{}), n: perDomain}
}

func (l *domainLimiter) acquire(rawURL string) {
	host := extractRootDomain(rawURL)
	l.mu.Lock()
	sem, ok := l.sems[host]
	if !ok {
...
func FetchAll(urls []string, globalConcurrency, perDomainConcurrency int) ([]Result, *Stats) {
	...
	for i, u := range urls {
		...
		go func(idx int, subURL string) {
			...
			res := Result{URL: subURL, Links: links, Err: err, ErrKind: kind}

			// Извлекаем preview из NoNodesErr
			if err != nil {
				var nnErr *parser.NoNodesErr
				if errors.As(err, &nnErr) {
					res.BodyPreview = nnErr.Preview
					res.RawPreview = nnErr.RawPreview
					res.ContentType = nnErr.ContentType
				}
...
func fetchWithRetry(subURL string, maxRetries int) ([]string, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		kind := classifyError(err)
		// Не ретраим: клиентские ошибки, DNS, "нет нод" — они не исправятся сами
